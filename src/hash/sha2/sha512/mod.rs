...
impl Sha512 {
    ...
    pub fn finalize(mut self) -> [u8; Self::DIGEST_LEN] {
        ...
        let plen_bits = 1024 - (mlen_bits + 128 + 1) % 1024 + 1; // pad len, in bits
        ...
        if cfg!(debug_assertions) {
            ...
            let dlen = mlen + plen + 16;
            debug_assert_eq!(dlen % Self::BLOCK_LEN as u128, 0);
        }
        ...
        // NOTE: pad len 大小会在 1.. 256 bytes 之内，所以可以安全的 unwrap。
        let plen = usize::try_from(plen).unwrap();

        let mlen_octets = u128::try_from(mlen_bits).unwrap();
        padding_block[plen..plen + 16].copy_from_slice(&mlen_octets.to_be_bytes());
        
        let data = &padding_block[..plen + 16];
        self.update(data);
